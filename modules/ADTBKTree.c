...
BKTree bk_create(MatchType type) {
    ...
    // If type is MT_EDIT_DIST we devide it to 28 smaller trees to reduce search time.
...
int bk_find(BKTree bktree, Map map_result, List complete_queries, CompareFunc compare_query, String word, int n) {
    ...
    if (bktree->type == MT_EDIT_DIST) {
        ...
        // In case min is lower than first pos of array
        int min = pos - (n);
        if( min < 0)
            min = 0;

        // In case max is higher than last pos of array
        int max = pos + (n);
        if(max > 27)
            max = 27;

        for(int i = min; i <= max; i++){
            if (bktree->root[i] == NULL)
                continue;
            find(bktree->root[i], bktree->compare, compare_query, map_result, complete_queries, word, n);
        }
        return 0;
    }
    // If type is MT_HAMMING_DIST.
