...
- insert(BKNode bkparent, BKNode new, CompareFunc compare)\ At first, this function will calculate the distance (d) betweeen the new node and the current node (parent node). After that if current node has no children, new node will be his newborn child. If parent has children,
