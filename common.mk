...
LIBRARY=core
TEST_DRIVER := $(MY_PATH)test_driver
CORE := $(MY_PATH)core
...
# compiler
CC = gcc  -fno-stack-protector -lpthread -pthread
CXX = g++ -lpthread -pthread
...
#
$(PROGS): $$($$@_OBJS)
ifneq ($(findstring test_through,$(PROGS)),test_through)
# ifneq '$(PROGS)' "test_through"
	$(CC)  $^ -o $@ $(LDFLAGS)

else
	$(CXX) $^ -shared -o lib$(LIBRARY).so
	$(CXX)  $^ -o $@ $(LDFLAGS) ./lib$(LIBRARY).so
endif
...
valgrind-%: %
	...
	valgrind -s --leak-check=full ./$* $($*_ARGS)
